...
Jeśli posiadasz już własne specyfikacje API, możesz załadować je do Akamai API Security — zarówno przez interfejs użytkownika (UI), jak i za pomocą Akamai API Security Management API. Dzięki temu narzędzie może porównać (wykonać diff) to, co według powinno znajdować się w specyfikacji, z tym, co faktycznie obserwujemy w środowisku produkcyjnym.
...
Teraz możemy szybko sprawdzić, że spośród dziesiątek naszych interfejsów API jeden zwraca numer karty (Card Number) w treści odpowiedzi. Po otwarciu szczegółów tego API możemy przewinąć w dół i dokładnie zobaczyć, w którym miejscu odpowiedzi ten numer karty jest zwracany.

15. Otwórz przykład wywołania API klikając `Sample` i znajdź numer karty kredytowej. 
...
1. Przejdź do strony `Overwiev` klikając zakładkę **Security**.
...
Widok szczegółów zawiera czytelne informacje o tym: co się wydarzyło, dlaczego stanowi to problem oraz jakie kroki naprawcze należy podjąć (remediation steps).

5. Kliknije przycisk `Evidence`.

![](./images/APISec_18.png)

Evidence (Dowody) pozwala przejrzeć dowody analizy śledczej (forensic evidence) związane z danym przypadkiem.
...
Jedną z zalet wykonywania tego działania bezpośrednio z poziomu Akamai API Security jest to, że rozwiązanie to jest niezależne od używanych narzędzi (tool-agnostic), dzięki czemu może blokować atakującego na wielu warstwach jednocześnie — w API Gatewayach i WAF-ach. Co ważne, wszystko to można zrobić w ramach jednej, spójnej platformy.
